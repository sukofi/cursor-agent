...
1. `.env` に `DISCORD_BOT_TOKEN=...` を設定（Webhook も使う場合は `DISCORD_WEBHOOK_SKILLS_LIST` などを追加）
...
| `mcp_client.py` | MCP クライアント（.env の `MCP_SERVER_CMD` または `project/mcp_servers.json` で連携） |
| `check_mcp.py` | MCP 接続の事前確認スクリプト |
